...
apro_files <- list.files(paste0(params$outdir, 'phylogeny/',
...
```{r cxx_models}
times_df <- read_delim("tmp.txt", 
                       col_names = c("basename","s","h:m:s","max_rss",
                                     "max_vms","max_uss","max_pss",
                                     "io_in","io_out","mean_load","cpu_time")) %>% 
  mutate(max_rss_gb = max_rss/1024, basename=gsub(".txt", "", basename)) %>% 
  separate(basename, c("seed", "gene", "targets", "alphabet", "model"), sep = "_") %>% 
